...
trait F10LECA_Admin_Appearance_Trait
{
    public function sanitize_appearance(array $input): array
    {
        $defaults = F10LECA_Config::appearance_defaults();
        $current = F10LECA_Config::get_appearance();
        $value = wp_parse_args($current, $defaults);
        $presets = F10LECA_Config::appearance_presets();
    ...
    public function render_appearance_page(): void
    {
        ...
        <div class="wrap f10leca-admin-page">
            ...
            <nav class="nav-tab-wrapper f10leca-appearance-tabs">
                <a href="<?php echo esc_url(admin_url('admin.php?page=f10leca-lead-appearance&tab=form')); ?>" class="nav-tab <?php echo $tab === 'form' ? 'nav-tab-active' : ''; ?>">Formulário</a>
                <a href="<?php echo esc_url(admin_url('admin.php?page=f10leca-lead-appearance&tab=conversion')); ?>" class="nav-tab <?php echo $tab === 'conversion' ? 'nav-tab-active' : ''; ?>">Pós-conversão</a>
            </nav>

            <form method="post" action="options.php" data-f10leca-appearance-form data-f10leca-appearance-tab="<?php echo esc_attr($tab); ?>">
                <?php settings_fields('f10leca_appearance_group'); ?>

                <?php if ($tab === 'form') : ?>
                    <div class="f10leca-admin-layout">
                        <div class="f10leca-admin-settings-column">
                            <section class="f10leca-admin-card">
                                ...
                                <div class="f10leca-preset-grid">
                                    <?php foreach ($presets as $preset_key => $preset) : ?>
                                        <label class="f10leca-preset-card">
                                            <input type="radio" name="<?php echo esc_attr(self::APPEARANCE_OPTION); ?>[preset]" value="<?php echo esc_attr($preset_key); ?>" <?php checked($appearance['preset'], $preset_key); ?> data-f10leca-preset>
                                            <span class="f10leca-preset-card__swatches" aria-hidden="true"><span style="background:<?php echo esc_attr((string) $preset['settings']['form_background']); ?>"></span><span style="background:<?php echo esc_attr((string) $preset['settings']['button_background']); ?>"></span><span style="background:<?php echo esc_attr((string) $preset['settings']['title_color']); ?>"></span></span>
                                            <strong><?php echo esc_html((string) $preset['label']); ?></strong><small><?php echo esc_html((string) $preset['description']); ?></small>
                                        </label>
                                    <?php endforeach; ?>
                                </div>
                            </section>
                            <section class="f10leca-admin-card"><h2>Estrutura e responsividade</h2><div class="f10leca-control-grid">
                            ...
                            <section class="f10leca-admin-card"><h2>Cores</h2><div class="f10leca-control-grid f10leca-control-grid--colors">
                            ...
                            <section class="f10leca-admin-card"><h2>Acabamento e tipografia</h2><div class="f10leca-control-grid">
                        ...
                        <aside class="f10leca-admin-preview-column"><?php $this->render_form_appearance_preview(); ?></aside>
                    </div>
                <?php else : ?>
                    <div class="f10leca-admin-layout f10leca-admin-layout--conversion">
                        <div class="f10leca-admin-settings-column">
                            <section class="f10leca-admin-card"><h2>Caixa de pós-conversão</h2><div class="f10leca-control-grid">
                            ...
                            <p class="description">Os textos e o destino da pós-conversão são configurados individualmente em <a href="<?php echo esc_url(admin_url('admin.php?page=f10leca-lead-forms')); ?>">Formulários</a>.</p>
                            <?php submit_button('Salvar aparência da pós-conversão'); ?>
                        </div>
                        <aside class="f10leca-admin-preview-column"><?php $this->render_conversion_appearance_preview(); ?></aside>
                    </div>
                <?php endif; ?>
            </form>
        </div>
    ...
    {
        ?>
        <div class="f10leca-preview-toolbar"><strong>Pré-visualização</strong><div class="f10leca-device-switcher" role="group" aria-label="Tamanho da pré-visualização"><button type="button" class="button is-active" data-f10leca-device="desktop">Desktop</button><button type="button" class="button" data-f10leca-device="mobile">Mobile</button></div></div>
        <div class="f10leca-preview-stage" data-f10leca-preview-stage><div class="f10leca" data-f10leca-preview-form><div class="f10leca__header"><h2 class="f10leca__title">Fale com um especialista</h2><p class="f10leca__description">Preencha seus dados e a equipe entrará em contato.</p></div><div class="f10leca__grid"><label class="f10leca__field"><span>Nome</span><input type="text" value="Jeferson Marques" readonly></label><label class="f10leca__field"><span>WhatsApp</span><input type="text" value="(41) 99999-9999" readonly></label><label class="f10leca__field"><span>E-mail</span><input type="text" value="contato@exemplo.com.br" readonly></label><label class="f10leca__field"><span>Escola ou empresa</span><input type="text" value="Escola Exemplo" readonly></label></div><button class="f10leca__button" type="button">Quero saber mais</button></div></div>
        <?php
    }
    ...
    {
        ?>
        <div class="f10leca-preview-toolbar"><strong>Pré-visualização</strong></div><div class="f10leca-preview-stage f10leca-preview-stage--conversion" data-f10leca-preview-stage><div class="f10leca f10leca--align-center f10leca-preview-conversion-shell" data-f10leca-preview-form><div class="f10leca__conversion is-visible"><span class="f10leca__conversion-icon dashicons dashicons-yes-alt" aria-hidden="true"></span><h3 class="f10leca__conversion-title">Seu conteúdo está pronto</h3><p class="f10leca__conversion-description">Clique no botão abaixo para continuar.</p><a class="f10leca__conversion-button" href="#">Baixar material</a></div></div></div>
        <?php
    }

    private function render_color_control(string $label, string $key, array $appearance): void
    {
        ?><label class="f10leca-control"><span><?php echo esc_html($label); ?></span><span class="f10leca-color-control"><input type="color" value="<?php echo esc_attr((string) $appearance[$key]); ?>" data-f10leca-color-picker="<?php echo esc_attr($key); ?>"><input type="text" name="<?php echo esc_attr(self::APPEARANCE_OPTION); ?>[<?php echo esc_attr($key); ?>]" value="<?php echo esc_attr((string) $appearance[$key]); ?>" maxlength="7" data-f10leca-appearance-setting="<?php echo esc_attr($key); ?>"></span></label><?php
    }

    private function render_number_control(string $label, string $key, array $appearance, int $min, int $max, string $suffix): void
    {
        ?><label class="f10leca-control"><span><?php echo esc_html($label); ?></span><span class="f10leca-number-control"><input type="number" min="<?php echo esc_attr((string) $min); ?>" max="<?php echo esc_attr((string) $max); ?>" name="<?php echo esc_attr(self::APPEARANCE_OPTION); ?>[<?php echo esc_attr($key); ?>]" value="<?php echo esc_attr((string) $appearance[$key]); ?>" data-f10leca-appearance-setting="<?php echo esc_attr($key); ?>"><small><?php echo esc_html($suffix); ?></small></span></label><?php
    }

    private function render_select_control(string $label, string $key, array $appearance, array $options): void
    {
        ?><label class="f10leca-control"><span><?php echo esc_html($label); ?></span><select name="<?php echo esc_attr(self::APPEARANCE_OPTION); ?>[<?php echo esc_attr($key); ?>]" data-f10leca-appearance-setting="<?php echo esc_attr($key); ?>"><?php foreach ($options as $value => $option_label) : ?><option value="<?php echo esc_attr((string) $value); ?>" <?php selected((string) $appearance[$key], (string) $value); ?>><?php echo esc_html((string) $option_label); ?></option><?php endforeach; ?></select></label><?php
    }
}
