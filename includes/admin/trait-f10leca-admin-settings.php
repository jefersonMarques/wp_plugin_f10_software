...
trait F10LECA_Admin_Settings_Trait
{
    public function sanitize_settings(array $input): array
    {
        $current = $this->get_settings();
        $token_input = isset($input['f10leca_token']) ? trim((string) $input['f10leca_token']) : '';
        $api_key_input = isset($input['brevo_api_key']) ? trim((string) $input['brevo_api_key']) : '';

        $f10leca_token = !empty($input['clear_f10leca_token'])
            ? ''
            : ($token_input !== '' ? sanitize_text_field($token_input) : (string) $current['f10leca_token']);
        ...
        return array(
            'f10leca_enabled' => !empty($input['f10leca_enabled']) ? '1' : '0',
            'f10leca_token' => $f10leca_token,
            'f10leca_unit_id' => isset($input['f10leca_unit_id']) ? (string) absint($input['f10leca_unit_id']) : '',
            'f10leca_source' => isset($input['f10leca_source']) ? sanitize_text_field((string) $input['f10leca_source']) : '',
            'f10leca_media' => isset($input['f10leca_media']) ? sanitize_text_field((string) $input['f10leca_media']) : '',
    ...
    public function render_settings_page(): void
    {
        ...
        $jwt_status = F10LECA_Integrations::inspect_jwt((string) $settings['f10leca_token']);
        ...
        <div class="wrap">
            ...
            <form method="post" action="options.php">
                <?php settings_fields('f10leca_settings_group'); ?>
                ...
                <table class="form-table" role="presentation">
                    <tr><th scope="row">Ativar envio para F10</th><td><label><input type="checkbox" name="<?php echo esc_attr(self::OPTION_NAME); ?>[f10leca_enabled]" value="1" <?php checked($settings['f10leca_enabled'], '1'); ?>> Enviar cada novo lead para a API da F10</label></td></tr>
                    <tr><th scope="row">URL da API F10</th><td><code><?php echo esc_html(F10LECA_Config::F10_ENDPOINT); ?></code><p class="description">Endpoint fixo utilizado automaticamente pelo plugin.</p></td></tr>
                    <tr>
                        <th scope="row"><label for="f10leca_token">Token JWT F10</label> <?php echo wp_kses_post($this->render_help_icon('Solicitar à equipe F10 o token para envio de leads.')); ?></th>
                        <td>
                            <input id="f10leca_token" class="regular-text code" type="password" name="<?php echo esc_attr(self::OPTION_NAME); ?>[f10leca_token]" value="" autocomplete="new-password" placeholder="<?php echo esc_attr($settings['f10leca_token'] ? 'Token configurado — deixe em branco para manter' : 'Informe o token JWT'); ?>">
                            <?php if ((string) $settings['f10leca_token'] !== '') : ?><p class="description">Token salvo: <code><?php echo esc_html($this->mask_secret((string) $settings['f10leca_token'])); ?></code></p><?php endif; ?>
                            <label style="display:block;margin-top:8px"><input type="checkbox" name="<?php echo esc_attr(self::OPTION_NAME); ?>[clear_f10leca_token]" value="1"> Remover o token salvo</label>
                            <p class="description"><?php echo esc_html($this->jwt_status_message($jwt_status)); ?></p>
                        </td>
                    </tr>
                    <tr><th scope="row"><label for="f10leca_unit_id">ID da unidade</label> <?php echo wp_kses_post($this->render_help_icon('Solicitar à equipe F10 o ID da sua unidade.')); ?></th><td><input id="f10leca_unit_id" type="number" min="1" name="<?php echo esc_attr(self::OPTION_NAME); ?>[f10leca_unit_id]" value="<?php echo esc_attr((string) $settings['f10leca_unit_id']); ?>"></td></tr>
                    <tr><th scope="row"><label for="f10leca_source">Fonte</label> <?php echo wp_kses_post($this->render_help_icon('Consulte como configurar fontes e eventos no F10.', $help_url)); ?></th><td><input id="f10leca_source" class="regular-text" type="text" name="<?php echo esc_attr(self::OPTION_NAME); ?>[f10leca_source]" value="<?php echo esc_attr((string) $settings['f10leca_source']); ?>" placeholder="Ex.: Site F10"><p class="description">Mesmo texto informado no F10. <a href="<?php echo esc_url($help_url); ?>" target="_blank" rel="noopener noreferrer">Ver ajuda sobre fontes e mídias</a>.</p></td></tr>
                    <tr><th scope="row"><label for="f10leca_media">Mídia</label> <?php echo wp_kses_post($this->render_help_icon('Consulte como configurar fontes e eventos no F10.', $help_url)); ?></th><td><input id="f10leca_media" class="regular-text" type="text" name="<?php echo esc_attr(self::OPTION_NAME); ?>[f10leca_media]" value="<?php echo esc_attr((string) $settings['f10leca_media']); ?>" placeholder="Ex.: Site F10"><p class="description">Mesmo texto informado no F10. <a href="<?php echo esc_url($help_url); ?>" target="_blank" rel="noopener noreferrer">Ver ajuda sobre fontes e mídias</a>.</p></td></tr>
                </table>
            ...
            <p>Os textos, campos e ações após o envio são configurados em <a href="<?php echo esc_url(admin_url('admin.php?page=f10leca-lead-forms')); ?>">Leads F10 → Formulários</a>.</p>
        </div>
        <?php
    }
