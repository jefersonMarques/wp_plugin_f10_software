...
trait F10_Lead_Capture_Admin_Leads_Trait
{
    ...
    public function handle_export(): void
    {
        ...
        $csv_header = $this->build_csv_line(
            array(
                'ID', 'Data', 'Formulário', 'ID do formulário', 'Nome', 'Telefone', 'WhatsApp', 'E-mail', 'Escola/empresa', 'Produto', 'Observações',
                'Origem', 'Suborigem', 'Página', 'Referência', 'UTM Source', 'UTM Medium',
                'UTM Campaign', 'UTM Term', 'UTM Content', 'Status', 'F10', 'Brevo', 'Tentativas',
                'Ação pós-conversão', 'Status da ação', 'Texto da ação', 'URL da ação', 'Acionamentos', 'Primeiro acionamento',
            )
        );

        // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Conteúdo não HTML; build_csv_line aplica o escape específico do formato CSV.
        echo $csv_header;

        foreach ($leads as $lead) {
            $csv_line = $this->build_csv_line(
                array(
                    $lead['id'],
                    $this->format_date((string) $lead['created_at']),
                    $this->lead_form_name((string) $lead['form_id']),
                    $lead['form_id'],
                    $lead['name'],
                    $lead['phone'],
                    $lead['whatsapp'],
                    ...
                    $lead['conversion_status'] ?? 'none',
                    $lead['conversion_label'] ?? '',
                    $lead['conversion_url'] ?? '',
            ...
            // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Conteúdo não HTML; build_csv_line escapa aspas, delimitadores, quebras e fórmulas de planilha.
            echo $csv_line;
        }
    ...
    private function render_lead_details(int $lead_id): void
    {
        ...
        $fields = array(
            ...
            'Formulário' => $this->lead_form_name((string) $lead['form_id']),
            ...
            'Texto da ação' => $lead['conversion_label'] ?? '',
            'URL da ação' => $lead['conversion_url'] ?? '',
            ...
            <table class="widefat striped" style="max-width:1000px">
                <tbody>
                    <?php foreach ($fields as $label => $value) : ?>
                        <tr>
                            <th style="width:240px"><?php echo esc_html($label); ?></th>
