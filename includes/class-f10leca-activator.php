...
final class F10LECA_Activator
{
    private const DB_VERSION = '1.5.0';
    private const DB_VERSION_OPTION = 'f10leca_db_version';
    ...
    public static function activate(): void
    {
        ...
        F10LECA_Integrations::reconcile_stored_f10leca_results();
    ...
    private static function create_table(): void
    {
        ...
        $sql = "CREATE TABLE {$table_name} (
            ...
            f10leca_status varchar(20) NOT NULL DEFAULT 'pending',
            brevo_status varchar(20) NOT NULL DEFAULT 'pending',
            f10leca_http_status smallint(5) unsigned NULL,
            brevo_http_status smallint(5) unsigned NULL,
            f10leca_response longtext NULL,
    ...
    {
        $option_name = 'f10leca_settings';
        $current = get_option($option_name, null);
        $settings = wp_parse_args(
            is_array($current) ? $current : array(),
            F10LECA_Config::default_settings()
        );

        unset($settings['f10leca_url']);
        ...
        self::ensure_option(
            'f10leca_appearance',
            F10LECA_Config::appearance_defaults()
        );
    }
    ...
    {
        $current = get_option(F10LECA_Config::FORMS_OPTION, null);

        if (is_array($current) && !empty($current)) {
            F10LECA_Config::save_forms($current);
            return;
        }

        $default = F10LECA_Config::default_form(
            F10LECA_Config::get_settings(),
            (array) get_option('f10leca_conversion', array())
        );

        add_option(
            F10LECA_Config::FORMS_OPTION,
    ...
    {
        $current = get_option(F10LECA_WhatsApp_Config::OPTION_NAME, null);

        if ($current === null) {
            add_option(F10LECA_WhatsApp_Config::OPTION_NAME, array(), '', false);
            return;
        }

        if (is_array($current)) {
            F10LECA_WhatsApp_Config::save_widgets($current);
        }
    }
    ...
    {
        if (!wp_next_scheduled('f10leca_retry_event')) {
            wp_schedule_event(time() + 300, 'hourly', 'f10leca_retry_event');
        }
    }
}
